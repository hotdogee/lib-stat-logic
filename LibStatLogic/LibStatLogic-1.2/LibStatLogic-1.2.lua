...
Last Update: $Date: 2014-09-01 09:06:06 +0800 (Mon, 01 Sep 2014) $
Website:
Documentation:
SVN: $URL $
Dependencies: UTF8
...
--[[
	...
	5.4.8		18414  --Siege of Orgrimmar GetBuildINfo() -> "5.4.8", "18414", "Jun 13 2014",  50400  Released to live 5/20/2014
--]]
...
--Called by /sl analyze [itemID]  (e.g. /sl analyze 105807)
local function AnalyzeItem(itemString)
	StatLogic:SetTip("item:"..itemString)
end;

--Called by /sl unrecognized
local function DumpUnrecognizedItems()
	if (StatLogic.ItemsNotRecognized == nil) then
		print("There are no unrecognized attributes to report");
		return;
	end;
...
--Called by /sl parse [string] (E.g. /sl parse 5,399 Armor)
...
Notes:
  ...
  * rating:
  :;RatingID : number - A CR_xxx combat rating ID constant as defined in PaperDollFrame.lua of Blizzard default ui
...
-- 1, 2, 3: Appropriate armor in all slots, then its your GetSpecialization()
...
local H = {
  ...
  [90] = 48.3333177690963, --TODO: power-law extension of 80-86. Figure out real level 90 value
...
local combatRatingBonus = {
	...
	[85] = {
		...
		[CR_PVP_POWER] = 120, --PvP Power Rating / PvP Power % (estimated)
	},
	[86] = {
		...
		[CR_PVP_POWER] = 152, --PvP Power Rating / PvP Power % (estimated)
	},
	[87] = {
		--comes from linear regressions of Rating -> Bonus
		...
		[CR_PVP_POWER] = 194, --PvP Power Rating / PvP Power % (estimated)
	},
	[88] = {
		--comes from linear regressions of Rating -> Bonus
		...
		[CR_PVP_POWER] = 247, --PvP Power Rating / PvP Power % (estimated)
	},
	[89] = {
		--comes from linear regressions of Rating -> Bonus
		...
		[CR_PVP_POWER] = 315, --PvP Power Rating / PvP Power % (estimated)
	},	
	[90] = {
		--comes from linear regressions of Rating -> Bonus
		...
		[CR_PVP_POWER] = 400.0000, --PvP Power Rating / PvP Power %
	},
...
function StatLogic:GetEffectFromRating(rating, id, level, class)
	...
	debugPrint(string.format("combatUnitRatingBase=%d, CombatRatingID=%d", combatUnitRatingBase, id));
	...
	if (level > 85) then
		...
		if (not ratingPerBonus) then
			...
			debugPrint(string.format("combatUnitRatingBase[%s] = %s", RatingNameToID[id], combatUnitRatingBase));
			debugPrint(string.format("hvalue = %s", hvalue));
